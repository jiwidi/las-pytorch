...
def spec_augment(
    mel_spectrogram, frequency_mask_num=1, time_mask_num=2, frequency_masking_para=5, time_masking_para=15
):
...
def concat_and_subsample(features, left_frames=3, right_frames=0, skip_frames=2):
    ...
    concated_features = np.zeros(shape=[time_steps, (1 + left_frames + right_frames) * feature_dim], dtype=np.float32)

    concated_features[:, left_frames * feature_dim : (left_frames + 1) * feature_dim] = features

    for i in range(left_frames):
        concated_features[
            i + 1 : time_steps, (left_frames - i - 1) * feature_dim : (left_frames - i) * feature_dim
        ] = features[0 : time_steps - i - 1, :]

    for i in range(right_frames):
        concated_features[
            0 : time_steps - i - 1, (right_frames + i + 1) * feature_dim : (right_frames + i + 2) * feature_dim
        ] = features[i + 1 : time_steps, :]
...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.left_frames > 0 or self.right_frames > 0:
            feature = concat_and_subsample(
                feature, left_frames=self.left_frames, right_frames=self.right_frames, skip_frames=self.skip_frames
            )
...
def collate_fn_with_eos_bos(batch):
    ...
    for _, feat, feat_len, target, target_len in batch:
        padded_features.append(
            np.pad(feat, ((0, max_feature_length - feat_len), (0, 0)), mode="constant", constant_values=0.0)
        )
        padded_targets.append([BOS] + target + [EOS] + [PAD] * (max_target_length - target_len))
...
class AudioDataLoader(object):
    def __init__(self, dataset, shuffle=False, ngpu=1, mode="ddp", include_eos_sos=False, num_workers=8):
        if ngpu > 1:
            if mode == "ddp":
